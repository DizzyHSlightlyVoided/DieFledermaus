...
* `RsaSig` - *One parameter.* The stream is digitally signed with RSA, using an SHA-512 hash (or SHA-3/512 fi `SHA3` is enabled) of the uncompressed data. In the DieFledermaus class library, this is accomplished using BouncyCastle's `Org.BouncyCastle.Crypto.Signers.RsaDigestSigner` class, which appears to use PKCS #1.5 padding. If the DieFledermaus stream is encrypted, an encoder should always store this in the **Encrypted Format**, not in the plaintext format.
