...
* `RsaSig` - *One parameter.* **RSA Sig**niert, or **RSA Sig**ned. The stream is digitally signed with an RSA private key, using an SHA-512 hash (or SHA-3/512 fi `SHA3` is enabled) of the uncompressed data. The signature may be verified using the corresponding RSA public key. In the DieFledermaus class library, this is accomplished using BouncyCastle's `Org.BouncyCastle.Crypto.Signers.RsaDigestSigner` class, which appears to use PKCS #1.5 padding. If the DieFledermaus stream is encrypted, an encoder should always store this in the **Encrypted Format**, not in the plaintext format.
* `RSASch` - *One parameter.* **RSA Sch**lüssel ("RSA key"). Usable only if `AES` is also present. The encryption key is encrypted using an RSA public key, and is transmitted as the parameter. The parameter is then decrypted using the corresponding RSA private key. The RSA format used is equivalent to the .Net Framework [`RSACryptoServiceProvider.Encrypt()`](https://msdn.microsoft.com/en-us/library/system.security.cryptography.rsacryptoserviceprovider.encrypt%28v=vs.110%29.aspx) method, which uses PKCS#1 v1.5 padding.
