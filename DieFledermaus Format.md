...
* `Hash` - *1 parameter.* Indicates the specified hash function. Must be in plaintext. Valid values of the parameter are one of the following UTF-8 strings:
 - `SHA224`
...
* `RSAsch` - *version 1, one paramter.* "**RSA Sch**lüssel" ("RSA key"). The encoder encrypts the binary key using an RSA public key, and is transmitted as the parameter. The decoder then uses the corresponding RSA private key to decrypt the key. Must be in plaintext.
