...
* `DeL` - *version 1, one parameter.* **De**compressed **L**ength, or **De**komprimierte **L**änge. The parameter is a signed 64-bit integer containing the number of bytes in the uncompressed data. If the archive is not encrypted, this value must be equal to **Decompressed Length**. Should be encrypted.
...
* `ECDSAsig` - *version 1, one or two parameters.* Same as `DSAsig`, but using the [ECDSA](https://en.wikipedia.org/wiki/Elliptic_Curve_Digital_Signature_Algorithm) algorithm. Must be in **Primary Format**.
* `RSAsch` - *version 1, one paramter.* "**RSA Sch**lüssel" ("RSA key"). The encoder encrypts the binary key using an RSA public key, and is transmitted as the parameter. The decoder then uses the corresponding RSA private key to decrypt the key. Must be in **Primary Format**.
