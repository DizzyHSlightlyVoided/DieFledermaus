...
* **Entry Filename:** An 8-bit length-prefixed UTF-8 string specifying the full path of the entry within the DieFledermauZ archive's file structure.
...
### Entry ID
The **Entry ID** must be unique, sequential, and start at 0. That is, if there are four entries, they must collectively have the IDs 0, 1, 2, and 3. However, they are not required to be in any specific order. (An encoder should assign them the same order that they are written into the file, but this is because that's just the easiest way to do it.)
...
The other way is to encrypt the entire archive. This is indicated by giving **Options** the `Ver` option from DieFledermaus's **Primary Format**. As with DieFledermaus, an encoder should use a PBKDF2-encoded password, but a binary key may also be used. Everything after **Options** is encrypted, and the following plaintext fields are placed after **Options**:
...
If multiple entries are individually encrypted, an encoder should make every effort to ensure that each file's **Salt** and **IV** values are unique, especially when they have the same key or password. That said, the size of a key and IV are each 128 bits, so if they are generated by a properly random or pseudorandom number generator, the odds of a collision are 1 in 2<sup>256</sup>, or approximately 1 in 1.159&times;10<sup>77</sup> (that's a 77-digit number), so the danger isn't *too* big.

### Encrypted Filenames
If the filename is encrypted on an individual basis, the **Entry Filename** in both **Entry List** and **Offset List** must be stated as the string "//V" (`2f 2f 56`) followed by a textual representation of **Entry ID**, using ASCII digits and with no commas, leading zeroes, or separators (i.e. if the ID is 69105, use "//V69105", `2f 2f 56 36 39 31 30 35`). When listing files, a decoder should display something more sensible than this, i.e. "(Encrypted Entry at index 69105)".
...
The manifest must include every file listed in the archive *except* the manifest itself. It must not be compressed, encrypted (unless the entire archive is encrypted), or contain any other entries in its **Format** except the file path.
